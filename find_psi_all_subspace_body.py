...
for test_order in range(START_ORDER, END_ORDER+1):
    ...
    subspace, matrixrows = comm.fill_subspace_norm(Jpart, Hcomm, test_order)
    ...
    psi += psi_test
    try:
        normdict = comm.check_normalisable(psi, iofvars, test_order,
                                           orders, split_orders, zero_not_needed=True)
