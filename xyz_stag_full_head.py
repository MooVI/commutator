...
X, D, Y = symbols('X D Y', real=True)
comm.mathematica_parser.vardict = {'X': X, 'D':D, 'Y':Y}
L = 40
orders = {X:1,D:1}
p.orders = orders
Ypart = [N(I*((Y-D) if j%2==0 else (Y+D)), [2*j+1,2*j+4]) for j in range(L-2)]
