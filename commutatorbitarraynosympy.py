...
def fba_zeros(count):
    return frozenbitarray(bitarray.util.zeros(count))

def ba_zeros(count):
    return bitarray.util.zeros(count)

def leftshift(ba, count):
    return ba[count:] + fba_zeros(count)

def rightshift(ba, count):
    return fba_zeros(count) + ba[:-count]
...
def mathematica_parser(exprstring):
    return exprstring.replace('^', '**'), mathematica_parser.vardict
...
class NCProduct:
    ...
    def is_product(self):
        return self.product[0].any() or self.product[1].any()

    def is_identity(self):
        return not (self.product[0].any() or self.product[1].any())
    ...
    def hermitian_conjugate(self):
        return type(self)(self.scalar*self.reverse_sign(), self.product)

    def conjugate(self):
        return type(self)(self.scalar, self.product)
    ...
    def __mul__(self, other):
        ...
        if isinstance(other, typ):
            return typ(self.scalar*other.scalar*(-1)**((self.product[1]&other.product[0]).count())
                                                 , [self.product[0]^other.product[0],
                                                    self.product[1]^other.product[1]])
        elif isinstance(other, NCSum):
    ...
    def __rmul__(self, other):
        typ = type(self)
        if isinstance(other, typ):
            return typ(self.scalar*other.scalar*(-1)**((self.product[0]&other.product[1]).count())
                                                 , [self.product[0]^other.product[0],
                                                    self.product[1]^other.product[1]])
        elif isinstance(other, NCSum):
...
class Z2Product(NCProduct):
    ...
    def translate(self, nSites):
        return type(self)(self.scalar, [rightshift(self.product[0], nSites),
                                        rightshift(self.product[1], nSites)])
    ...
    def get_operator_on_site(self, site):
        mask = ba_zeros(L)
        mask[site-1] = 1
        return type(self)(1, [product[0]&mask, product[1]&mask])

    def support_indices(self):
        return list((i+1) for i in range(L) if self.product[0][i] or self.product[1][i])

    def support(self, L_periodic = None):
        site_indices = self.support_indices()
        if len(site_indices) < 2:
            return len(site_indices)
        if not L_periodic:
            return site_indices[-1]-site_indices[0]+1
...
class SigmaProduct(Z2Product):
    ...
    def self_stringify(self):
        ...
        strings = []
        nys = 0
        for a, (z, x) in enumerate(zip(self.product[0], self.product[1])):
            if x and z:
                strings.append('y' + str(a+1))
                nys += 1
            elif x:
                strings.append('x' + str(a+1))
            elif z:
                strings.append('z' + str(a+1))
        return strings, (1j)**nys
    ...
    #     for a, (z, x) in enumerate(zip(self.product[0], self.product[1])):
    #         if x and z:
    #             strings.append('\\sigma^y_{'  + str(a+1) + '}')
    #             scalar = scalar * I
    #         elif x:
    #             strings.append('\\sigma^x_{'  + str(a+1) + '}')
    #         elif z:
    #             strings.append('\\sigma^z_{'  + str(a+1) + '}')
    ...
    @staticmethod
    def stringify(product):
        strings = []
        for a, (z, x) in enumerate(zip(product[0], product[1])):
            if x and z:
                strings.append('y' + str(a+1))
            elif x:
                strings.append('x' + str(a+1))
    ...
    def destringify(string):
        ...
        for op in string.split(' '):
            if op[0] == 'z':
                ...
                print('Unknown operator ' + op)
        if nys %2 != 0:
            raise ValueError("Non-even number of ys!")
        return [frozenbitarray(zs),frozenbitarray(xs)], (-1)**(nys//2)
    ...
    def reverse_sign_product(product):
        ...
        for a pre-SORTED, NON-DUPLICATE product.
        """
        return 1-2*((product[0]&product[1]).count()%2)

    def reverse_sign(self):
        return 1-2*((self.product[0]&self.product[1]).count()%2)
...
def square_to_find_identity_scalar(group):
    ...
    for ncprod in collect_terms(group):
        result += (ncprod.scalar*ncprod.scalar)*ncprod.reverse_sign()
    return result
...
def trace_hermitian_square(group):
    result = 0
    for ncprod in collect_terms(group):
        result += (ncprod.scalar*ncprod.scalar)
    return result
...
def print_subspace(subspace, typ = SigmaProduct):
    for key, item in subspace.items():
        print(str(item)+ ': ' + typ.stringify(key))
