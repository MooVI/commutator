...
Vpart = [Ncproduct(V, [2*j+1,2*j+2,2*j+3,2*j+4]) for j in range(L-1)]
small =  Vpart+fpart
Jpart = [Ncproduct(I*Js[j%2], [2*j+2,2*j+3]) for j in range(L-1)]
H = Jpart+Vpart+fpart

START_ORDER = 1
END_ORDER = 4

START_PSI = [N(-I, 'a1 b1 a2')]
