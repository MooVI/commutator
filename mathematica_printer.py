...
class MathematicaStrPrinter(StrPrinter):
    """
    ...
    def _print_Function(self, expr):
        return (expr.func.__name__[0].upper()+expr.func.__name__[1:]
    ...
    def _print_MatrixBase(self, expr):
        if expr.rows == 0 or expr.cols == 0:
            return '%s, %s, {}' % (expr.rows, expr.cols)
        return "%s" % self._print(expr.tolist())
...
mathprinter = MathematicaStrPrinter()
...
def mstr(expr):
    ...
    'Eq(a + b, 0)'
    """
    return mathprinter.doprint(expr)
